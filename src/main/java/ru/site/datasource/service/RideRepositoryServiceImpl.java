...
public class RideRepositoryServiceImpl implements RideRepositoryService {
  ...
  public RideRepositoryServiceImpl(RideRepository rideRepository, RideMapper rideMapper) {
  ...
  public RideEntity getRide(Long id) {
    ...
    return rideRepository
        .findById(id)
        .orElseThrow(() -> new IllegalArgumentException("Поездка не найдена с ID: " + id));
  }
  ...
  public RideEntity saveRide(RideEntity ride) {
    if (ride.getId() != null) {
      Optional<RideEntity> existingRideOpt = rideRepository.findById(ride.getId());
