...
public class RideServiceImpl implements RideService {
  ...
  public RideServiceImpl(
      RideRepositoryService rideRepositoryService,
  ...
  public Ride getCurrentRideByClientId(Long clientId) {
    ...
    List<Ride> filteredRides =
        rides.stream()
            .filter(
                ride ->
                    RideStatus.IN_PROGRESS.equals(ride.getRideStatus())
                        || RideStatus.ASSIGNED.equals(ride.getRideStatus())
                        || RideStatus.REQUESTED.equals(ride.getRideStatus()))
  ...
  public void processDriverRideRequest(String message) throws JsonProcessingException {
    ...
    List<Ride> requetsedRides =
        rides.stream()
            .filter(ride -> ride.getRideStatus() == RideStatus.REQUESTED)
            .collect(Collectors.toList());

    List<Ride> sortedRides =
        requetsedRides.stream()
            .sorted(
                Comparator.comparingDouble(
                    ride ->
                        servicesUtil.calculateDistance(
                            ride.getStartLatitude(), ride.getStartLongitude(), lat, lon)))
            .collect(Collectors.toList());
    ...
    if (sortedRides.size() > 0) {
      ...
      DriverStatusChangedEvent driverStatusChangedEvent =
          DriverStatusChangedEvent.builder()
              .driverId(driverId)
